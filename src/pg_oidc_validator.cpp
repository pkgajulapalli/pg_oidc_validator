...
bool validate_token(const ValidatorModuleState* state, const char* token, const char* role,
  ...
  elog(LOG, "Found issuer_object: %s", serialize_map_to_string(issuer_object).c_str());
  ...
  set_config_option("pg_oidc_validator.oidc_token", token, PGC_USERSET, PGC_S_SESSION ,GUC_ACTION_SET, true, 0, false);
